...
import { Conteneur, EnTetePublique, EnTeteSection } from '@/components/public/sections'
...
import { Button, LienBouton, classesBouton } from '@/components/partages/button/button'
...
function PageAide() {
  ...
  return (
    <div>
      <EnTetePublique fond="noir" surtitre="Aide" titre="Tout ce qu’il faut savoir avant de miser.">
        {/* Ancre interne : `LienBouton` route, il ne sait pas viser un fragment de la page courante. */}
        <a href="#contact" className={classesBouton('volt', 'md')}>
      ...
      <Conteneur className="py-14 md:py-20">
        ...
        <div className="grid items-start gap-10 lg:grid-cols-[1fr_320px]">
          ...
          <div className="rounded-2xl bg-encre p-6 text-craie">
          ...
          <div className="rounded-2xl border border-trait bg-papier p-6">
      ...
      <section className="border-t border-trait bg-ardoise py-14 md:py-20">
        <Conteneur>
...
function FormulaireContact({ expediteur }: { expediteur: Expediteur | null }) {
  ...
  return (
    <form onSubmit={soumettre} noValidate className="overflow-hidden rounded-2xl border border-trait bg-papier">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-trait bg-gris px-5 py-3.5 sm:px-6">
      ...
      <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
        ...
        />
        {erreurGlobale && (
          <p className="rounded-xl border border-perte bg-perte-fond px-3.5 py-2.5 text-legende font-semibold text-perte sm:col-span-2" role="alert">
            {erreurGlobale}
          </p>
        )}
        {confirmation && !erreurGlobale && (
          <p className="rounded-xl border border-gain bg-gain-fond px-3.5 py-2.5 text-legende font-semibold text-gain sm:col-span-2" role="status">
            Message envoyé. Nous vous répondons à {confirmation}.
          </p>
        )}
      </div>
      <div className="mx-5 border-t border-trait sm:mx-6" />
